...
function plotExpression(expression) {
    ...
    if (!expression || expression.trim() === '') {
        statusMessage.textContent = "Please enter the function expression or the equation of the curve";
        return;
    }
    ...
    if (processedExpr.includes('=')) {
        ...
        if (parts.length !== 2) {
            statusMessage.textContent = "Invalid equation format. Please use the format f(x,y) = 0.";
            return;
        }
    ...
    try {
        ...
        statusMessage.textContent = `Expression error: ${error.message}`;
        return;
    }
...
function plotExplicitFunction(compiledExpr, left, right, originalExpression) {
    ...
    if (hasPoints) {
        ctx.stroke();
        statusMessage.textContent = `Plotting function: ${originalExpression}`;
    } else {
        statusMessage.textContent = "No valid points were found within the visible range.";
    }
}
...
function plotImplicitEquation(compiledExpr, left, right, top, bottom, originalExpression) {
    ...
    statusMessage.textContent = `Draw implicit equations: ${originalExpression}`;
}
...
function plotDerivativeExpression(expression) {
    ...
    if (parts.length === 1) {
        ...
        statusMessage.textContent = "Invalid format of the derivative expression";
        return;
    }
    ...
    try {
        ...
        if (hasPoints) {
            ...
            if (parts.length === 1) {
                statusMessage.textContent = `Plotting derivatives: derivative(${funcExpr})`;
            } else {
                statusMessage.textContent = `Plotting derivatives: derivative(${funcExpr}, ${variable})`;
            }
        } else {
            statusMessage.textContent = "No valid derivative value was found within the visible range.";
        }
    } catch (error) {
        statusMessage.textContent = `Error in differentiation: ${error.message}`;
    }
}
...
function plotIntegralExpression(expression) {
    ...
    if (parts.length === 1) {
        ...
        statusMessage.textContent = "Invalid format of the integral expression";
        return;
    }
    ...
    try {
        ...
        if (hasPoints) {
            ...
            if (parts.length === 1) {
                statusMessage.textContent = `Plotting integrals: integrate(${funcExpr}) (常数C=0)`;
            } else {
                statusMessage.textContent = `Plotting integrals: integrate(${funcExpr}, ${variable}) (常数C=0)`;
            }
        } else {
            statusMessage.textContent = "No valid score value was found within the visible range.";
        }
    } catch (error) {
        statusMessage.textContent = `Integral error: ${error.message}`;
    }
}
...
clearBtn.addEventListener('click', function() {
    ...
    drawGrid();
    statusMessage.textContent = "After entering the expression, the image will be displayed in real time.";
});
