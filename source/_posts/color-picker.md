...
categories:
keywords:
description: 一个可以提取屏幕任意位置的颜色的网页工具，基于 EyeDropper API，使用时请缩小浏览器窗口，并将你需要提取颜色的界面放置在浏览器窗口的外侧。
top_img:
cover:
---

* **屏幕颜色拾取器**：一个可以提取屏幕任意位置的颜色的网页工具，基于 EyeDropper API，使用时请缩小浏览器窗口，并将你需要提取颜色的界面放置在浏览器窗口的外侧。
...
<div class="color-picker-wrap">
  <div class="container">
    <header>
      ...
      <p class="subtitle">使用 EyeDropper API 提取屏幕任意位置的颜色，支持多种颜色格式转换。</p>
    </header>
        ...
        <div class="current-hex">
          ...
          <div class="hex-actions">
            <button class="btn btn-small case-toggle" id="toggleCase">
              <i class="fas fa-exchange-alt"></i> 大小写 </button>
            <button class="btn btn-small btn-success" id="copyHex">
              <i class="far fa-copy"></i> 复制 HEX </button>
          </div>
        </div>
        <button class="btn btn-primary" id="pickColor">
          <i class="fas fa-eye-dropper"></i> 打开颜色拾取器 </button>
        <p style="margin-top: 15px; color: #7f8c8d;"> 点击按钮后，鼠标会变成一个取色器，点击屏幕上任意位置即可获取颜色值。 </p>
      </div>
    </div>
    ...
    <div class="instructions">
      ...
      <p>3. 点击 HEX 颜色旁边的“大小写”按钮，可以切换 HEX 代码的大小写格式；</p>
      <p>4. 点击对应颜色格式的“复制”按钮，将颜色代码复制到剪贴板。</p>
    </div>
  </div>
  <div class="footer">
    <p>基于 <a href="https://github.com/WICG/eyedropper-api">EyeDropper API</a> 的屏幕颜色拾取工具 | 支持 HEX, RGB, HSL, HSV, CMYK 等多种格式</p>
    © <a href="https://github.com/huyongshuang/huyongshuang.github.io/tree/main/source/html/color-picker">GitHub</a>
  </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!window.EyeDropper) {
    ...
    pickColorBtn.innerHTML = '<i class="fas fa-exclamation-triangle"></i> 浏览器不支持 EyeDropper API';
    pickColorBtn.style.backgroundColor = '#e74c3c';
    showNotification('您的浏览器不支持 EyeDropper API，请使用 Chrome 或 Edge 浏览器', 'error');
  }
  ...
  copyHexBtn.addEventListener('click', () => {
    ...
    showNotification(`HEX 颜色 ${hexToCopy} 已复制到剪贴板`);
  });
